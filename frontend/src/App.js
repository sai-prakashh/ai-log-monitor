...
const Cursor = () => (
  <span style={{ display: "inline-block", width: 8, height: 13, background: "#58a6ff", marginLeft: 3, verticalAlign: "middle", animation: "blink 1s step-end infinite" }} />
);
      ...
      <div style={{ minHeight: "100vh", background: "#090c10", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'IBM Plex Sans', sans-serif", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", width: 380, background: "#0d1117", border: "1px solid #30363d", borderRadius: 8, padding: "36px 32px", boxShadow: "0 24px 64px #00000088", animation: "fadeSlideIn 0.4s ease" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* ✅ THE FIX: was <button onClick={...}> style={...} — stray > was closing the tag early */}
            <button
              onClick={() => isRegister ? register() : login()}
              style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", padding: "10px", borderRadius: 4, cursor: "pointer", background: "linear-gradient(135deg, #1f6feb, #388bfd)", border: "none", color: "#fff", marginTop: 4, transition: "opacity 0.15s" }}
            ...
            <p onClick={() => setIsRegister(!isRegister)} style={{ textAlign: "center", fontSize: 11, color: "#484f58", fontFamily: "'IBM Plex Mono',monospace", cursor: "pointer" }}
              ...
              <div className="card">
                ...
                <div style={{ padding: "12px 0", display: "flex", justifyContent: "center" }}>
                  <PieChart width={300} height={200}>
                    <Pie data={chartData} dataKey="value" outerRadius={72} innerRadius={36} paddingAngle={3}><Cell fill="#f85149" /><Cell fill="#1f6feb" /></Pie>
          ...
          <div>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
              ...
              {["", "error", "info", "warning"].map(f => (
                <button key={f} className={`btn ${filter === f ? "active" : ""}`} onClick={() => { setFilter(f); setPage(1); }}>{f === "" ? "ALL" : f.toUpperCase()}</button>
              ))}
            </div>
            <div className="card">
              ...
              <div style={{ maxHeight: 520, overflowY: "auto" }}>
                {logs.length === 0 ? (
                  <div style={{ padding: "40px 16px", textAlign: "center" }}><p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "#484f58" }}>waiting for logs<Cursor /></p></div>
                ) : logs.map((log, i) => (
          ...
          <div className="card">
            ...
            <div style={{ display: "flex", gap: 6, padding: "10px 12px", borderBottom: "1px solid #21262d" }}>
              {["", "CRITICAL", "ERROR", "WARNING"].map(f => (
                <button key={f} className={`btn ${alertFilter === f ? "active" : ""}`} onClick={() => setAlertFilter(f)} style={{ fontSize: 10, padding: "4px 10px" }}>{f === "" ? "ALL" : f}</button>
              ))}
            </div>
            <div style={{ maxHeight: 600, overflowY: "auto" }}>
              ...
              {alerts.filter(a => !alertFilter || a.severity === alertFilter).length === 0 ? (
                <div style={{ padding: "40px", textAlign: "center" }}><p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>no alerts detected</p></div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div className="card">
              ...
              <div className="service-row" style={{ background: "#161b22", borderBottom: "1px solid #21262d" }}>
                {["Service", "Total", "Errors", "Alerts", "Error Rate", "Error Bar"].map(h => <span key={h} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>)}
              </div>
              {services.length === 0 ? (
                <div style={{ padding: "32px", textAlign: "center" }}><p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>no service data yet</p></div>
              ) : services.map(s => {
        ...
        {activeTab === "timeseries" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div className="card">
              ...
              <div style={{ padding: "16px" }}>
                {timeseries.length === 0 ? (
                  <div style={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>collecting data<Cursor /></p></div>
                ) : (
                  <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={timeseries} margin={{ top: 4, right: 10, bottom: 4, left: 0 }}>
                      <defs>
                        <linearGradient id="gError" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f85149" stopOpacity={0.3} /><stop offset="95%" stopColor="#f85149" stopOpacity={0} /></linearGradient>
                        <linearGradient id="gAlert" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#e3b341" stopOpacity={0.3} /><stop offset="95%" stopColor="#e3b341" stopOpacity={0} /></linearGradient>
                      </defs>
                      <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
            ...
            <div className="card">
              ...
              <div style={{ padding: "16px" }}>
                {timeseries.length === 0 ? (
                  <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>collecting data<Cursor /></p></div>
                ) : (
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={timeseries.map(b => ({ ...b, total: (b.info || 0) + (b.error || 0) + (b.warning || 0) }))} margin={{ top: 4, right: 10, bottom: 4, left: 0 }}>
                      <defs>
                        <linearGradient id="gTotal" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#58a6ff" stopOpacity={0.3} /><stop offset="95%" stopColor="#58a6ff" stopOpacity={0} /></linearGradient>
                        <linearGradient id="gInfo"  x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3fb950" stopOpacity={0.2} /><stop offset="95%" stopColor="#3fb950" stopOpacity={0} /></linearGradient>
                      </defs>
                      <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
                      <XAxis dataKey="label" tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, fill: "#484f58" }} interval="preserveStartEnd" />
                      <YAxis tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, fill: "#484f58" }} />
