...
globalStyle.textContent = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .input-field {
    ...
    color: #e6edf3; padding: 9px 12px; width: 100%; outline: none;
  ...
  .pagination-btn {
    ...
    padding: 5px 12px; border-radius: 3px; border: 1px solid #30363d;
  ...
  .tab-bar {
    ...
    background: #0d1117; padding: 0 16px;
  }

  .service-row {
    display: grid; grid-template-columns: 140px 60px 60px 60px 80px 1fr;
    padding: 9px 16px; border-bottom: 1px solid #161b22;
...
const severityColor = s =>
  s === "CRITICAL" ? "#ff6b6b" : s === "ERROR" ? "#f85149" : s === "WARNING" ? "#e3b341" : "#8b949e";
...
const Cursor = () => (
  <span style={{
    ...
    background: "#58a6ff", marginLeft: 3, verticalAlign: "middle",
...
const StatCard = ({ label, value, dot, accent, sub }) => (
  <div className="stat-card">
    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
      ...
      <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#8b949e", textTransform: "uppercase", letterSpacing: "0.08em" }}>
        {label}
      </span>
    </div>
    <span className="count-anim" key={value} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 28, fontWeight: 600, color: accent, lineHeight: 1 }}>
      {value}
    </span>
    {sub && (
      <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58" }}>{sub}</span>
    )}
  </div>
);
...
function App() {
  ...
  const register = async () => {
    ...
    if (data.message) { alert("Registered ✅"); setIsRegister(false); }
  ...
  if (!token) {
    return (
      <div style={{ minHeight: "100vh", background: "#090c10", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'IBM Plex Sans', sans-serif", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", width: 380, background: "#0d1117", border: "1px solid #30363d", borderRadius: 8, padding: "36px 32px", boxShadow: "0 24px 64px #00000088", animation: "fadeSlideIn 0.4s ease" }}>
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ fontSize: 12, color: "#484f58", fontFamily: "'IBM Plex Mono',monospace" }}>{isRegister ? "// create account" : "authenticate to continue"}</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div>
              ...
              <input className="input-field" placeholder="user@domain.com" value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <div>
    ...
    <div style={{ minHeight: "100vh", background: "#090c10", fontFamily: "'IBM Plex Sans', sans-serif", color: "#e6edf3" }}>
      ...
      <nav style={{ background: "#0d1117", borderBottom: "1px solid #21262d", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 52, position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#3fb950" }}>LIVE</span>
          </div>
          {health && (
            <>
              <span style={{ width: 1, height: 16, background: "#21262d" }} />
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58" }}>
                {health.activeServices} services active
              </span>
            </>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          {liveCount > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#58a6ff12", border: "1px solid #58a6ff44", borderRadius: 4, padding: "4px 10px" }}>
              <div className="dot dot-blue" />
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#58a6ff", fontWeight: 600 }}>{liveCount} live alerts</span>
            </div>
          )}
          {criticalAlerts > 0 && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#f8514912", border: "1px solid #f8514944", borderRadius: 4, padding: "4px 10px" }}>
              <div className="dot dot-red" />
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#f85149", fontWeight: 600 }}>{criticalAlerts} CRITICAL</span>
            </div>
          )}
      ...
      <div className="tab-bar">
        {[
          ...
          <button key={t.id} className={`btn tab ${activeTab === t.id ? "active" : ""}`}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div className="card">
                <div className="section-header">
                  <div className="dot dot-blue" />
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Log Distribution</span>
                </div>
                <div style={{ padding: "12px 0", display: "flex", justifyContent: "center" }}>
                  <PieChart width={300} height={200}>
                    <Pie data={chartData} dataKey="value" outerRadius={72} innerRadius={36} paddingAngle={3}>
                      <Cell fill="#f85149" /><Cell fill="#1f6feb" />
                    </Pie>
                    <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 4, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11 }} itemStyle={{ color: "#e6edf3" }} />
                    <Legend formatter={v => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#8b949e" }}>{v}</span>} />
                  </PieChart>
                </div>
              </div>
              <div className="card">
                <div className="section-header">
                  <div className={criticalAlerts > 0 ? "dot dot-red" : "dot dot-inactive"} />
                  <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Top Critical Alerts</span>
                  <span style={{ marginLeft: "auto", fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, background: "#f8514922", color: "#f85149", border: "1px solid #f8514444", borderRadius: 2, padding: "1px 6px" }}>{criticalAlerts}</span>
                </div>
                <div style={{ maxHeight: 200, overflowY: "auto" }}>
                  {alerts.filter(a => a.severity === "CRITICAL").slice(0, 5).length === 0 ? (
                    <div style={{ padding: "32px 16px", textAlign: "center" }}>
                      <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#3fb950" }}>✓ no critical alerts</p>
                    </div>
                    ...
                    <div key={i} className="alert-row severity-critical" style={{ padding: "10px 14px", borderBottom: "1px solid #161b22" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                        <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, fontWeight: 600, color: "#ff6b6b" }}>CRITICAL</span>
                        <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, color: "#484f58" }}>{a.service}</span>
                      </div>
          ...
          <div>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
              <div style={{ position: "relative", flex: 1 }}>
                <span style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>$</span>
                <input className="input-field" placeholder="search logs..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} style={{ paddingLeft: 24 }} />
              </div>
              {["", "error", "info", "warning"].map(f => (
                <button key={f} className={`btn ${filter === f ? "active" : ""}`} onClick={() => { setFilter(f); setPage(1); }}>
                  {f === "" ? "ALL" : f.toUpperCase()}
                </button>
              ))}
            </div>
            <div className="card">
              <div className="section-header">
                <div className="dot dot-blue" />
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Event Stream</span>
                <span style={{ marginLeft: "auto", fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58" }}>page {page}/{pages}</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "60px 140px 1fr 150px", padding: "7px 16px", borderBottom: "1px solid #21262d", background: "#0d1117" }}>
                {["level", "service", "message", "time"].map(h => (
                  <span key={h} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
                ))}
              </div>
              <div style={{ maxHeight: 520, overflowY: "auto" }}>
                {logs.length === 0 ? (
                  <div style={{ padding: "40px 16px", textAlign: "center" }}>
                    <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 12, color: "#484f58" }}>waiting for logs<Cursor /></p>
                  </div>
                ) : logs.map((log, i) => (
                  <div key={log._id} className="log-row" style={{ display: "grid", gridTemplateColumns: "60px 140px 1fr 150px", padding: "9px 16px", borderBottom: "1px solid #161b22", animationDelay: `${Math.min(i, 10) * 0.03}s`, transition: "background 0.1s" }}
        ...
        {activeTab === "alerts" && (
          <div className="card">
            <div className="section-header">
              <div className={alerts.length > 0 ? "dot dot-red" : "dot dot-inactive"} />
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Alert Feed — sorted by severity</span>
              <span style={{ marginLeft: "auto", fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, background: "#f8514922", color: "#f85149", border: "1px solid #f8514444", borderRadius: 2, padding: "1px 6px" }}>{alerts.length}</span>
            </div>
            <div style={{ display: "flex", gap: 6, padding: "10px 12px", borderBottom: "1px solid #21262d" }}>
              {["", "CRITICAL", "ERROR", "WARNING"].map(f => (
                <button key={f} className={`btn ${alertFilter === f ? "active" : ""}`} onClick={() => setAlertFilter(f)} style={{ fontSize: 10, padding: "4px 10px" }}>
                  {f === "" ? "ALL" : f}
                </button>
              ))}
            </div>
            <div style={{ maxHeight: 600, overflowY: "auto" }}>
              <div style={{ display: "grid", gridTemplateColumns: "90px 130px 1fr 180px 160px", padding: "7px 16px", borderBottom: "1px solid #21262d", background: "#0d1117" }}>
                {["severity", "service", "message", "reason", "time"].map(h => (
                  <span key={h} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
                ))}
              </div>
              {alerts.filter(a => !alertFilter || a.severity === alertFilter).length === 0 ? (
                <div style={{ padding: "40px", textAlign: "center" }}>
                  <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>no alerts detected</p>
                </div>
              ) : alerts.filter(a => !alertFilter || a.severity === alertFilter).map((a, i) => (
                <div key={i} className={`alert-row severity-${(a.severity || "").toLowerCase()}`}
                  style={{ display: "grid", gridTemplateColumns: "90px 130px 1fr 180px 160px", padding: "10px 16px", borderBottom: "1px solid #161b22", animationDelay: `${Math.min(i, 8) * 0.04}s` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
              {services.slice(0, 4).map(s => (
                <div key={s.service} className="stat-card" style={{ borderLeft: `3px solid ${s.errors > 0 ? "#f85149" : "#3fb950"}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    <div className={`dot ${s.errors > 0 ? "dot-red" : "dot-green"}`} />
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#e6edf3", fontWeight: 600 }}>{s.service}</span>
                  </div>
                  <div style={{ display: "flex", gap: 12 }}>
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e" }}>logs: <span style={{ color: "#58a6ff" }}>{s.total}</span></span>
                    <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e" }}>err: <span style={{ color: "#f85149" }}>{s.errors}</span></span>
                  </div>
            ...
            <div className="card">
              <div className="section-header">
                ...
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Service Breakdown</span>
              </div>
              <div className="service-row" style={{ background: "#161b22", borderBottom: "1px solid #21262d" }}>
                {["Service", "Total", "Errors", "Alerts", "Error Rate", "Error Bar"].map(h => (
                  <span key={h} style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
                ))}
              </div>
              {services.length === 0 ? (
                <div style={{ padding: "32px", textAlign: "center" }}>
                  <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>no service data yet</p>
                </div>
              ) : services.map(s => {
                const rate = parseFloat(s.errorRate);
                const barColor = rate > 50 ? "#ff6b6b" : rate > 20 ? "#e3b341" : "#3fb950";
                return (
                  <div key={s.service} className="service-row">
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <div className={`dot ${s.errors > 0 ? "dot-red" : "dot-green"}`} />
                      <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#e6edf3" }}>{s.service}</span>
                    </div>
                    ...
                    <div style={{ paddingRight: 16 }}>
                      <div className="rate-bar-bg">
                        <div className="rate-bar-fill" style={{ width: `${Math.min(rate, 100)}%`, background: barColor }} />
                      </div>
                    </div>
                  </div>
                );
            ...
            <div className="card">
              <div className="section-header">
                <div className="dot dot-red" />
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Errors per Service</span>
              </div>
              <div style={{ padding: "16px" }}>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={services} margin={{ top: 4, right: 10, bottom: 4, left: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
                    <XAxis dataKey="service" tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, fill: "#484f58" }} />
                    <YAxis tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, fill: "#484f58" }} />
                    <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 4, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11 }} itemStyle={{ color: "#e6edf3" }} />
                    <Bar dataKey="errors" fill="#f85149" radius={[2, 2, 0, 0]} />
                    <Bar dataKey="alerts" fill="#e3b341" radius={[2, 2, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        {activeTab === "timeseries" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {[1, 5, 60].map(w => (
                <button key={w} className={`btn ${tsWindow === w ? "active" : ""}`} onClick={() => setTsWindow(w)}>
                  {w === 1 ? "1 min" : w === 5 ? "5 min" : "1 hour"}
                </button>
              ))}
              <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#484f58", marginLeft: 8 }}>
                {timeseries.length} buckets · auto-refresh 5s
              </span>
            </div>
            <div className="card">
              <div className="section-header">
                <div className="dot dot-red" />
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Errors &amp; Alerts Over Time</span>
              </div>
              <div style={{ padding: "16px" }}>
                {timeseries.length === 0 ? (
                  <div style={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <p style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#484f58" }}>collecting data<Cursor /></p>
                  </div>
                ) : (
                  <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={timeseries} margin={{ top: 4, right: 10, bottom: 4, left: 0 }}>
                      <defs>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke="#21262d" />
                      <XAxis dataKey="label" tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, fill: "#484f58" }} interval="preserveStartEnd" />
                      <YAxis tick={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 9, fill: "#484f58" }} />
                      <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 4, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11 }} itemStyle={{ color: "#e6edf3" }} />
                      <Legend formatter={v => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#8b949e" }}>{v}</span>} />
                      <Area type="monotone" dataKey="error"  stroke="#f85149" fill="url(#gError)" strokeWidth={2} dot={false} />
                      <Area type="monotone" dataKey="alerts" stroke="#e3b341" fill="url(#gAlert)" strokeWidth={2} dot={false} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="card">
              <div className="section-header">
                <div className="dot dot-blue" />
                <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 11, color: "#8b949e", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>Log Volume Over Time</span>
              </div>
              <div style={{ padding: "16px" }}>
                {timeseries.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={timeseries.map(b => ({ ...b, total: (b.info || 0) + (b.error || 0) + (b.warning || 0) }))} margin={{ top: 4, right: 10, bottom: 4, left: 0 }}>
                      <defs>
                      ...
                      <Tooltip contentStyle={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 4, fontFamily: "'IBM Plex Mono',monospace", fontSize: 11 }} itemStyle={{ color: "#e6edf3" }} />
                      <Legend formatter={v => <span style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10, color: "#8b949e" }}>{v}</span>} />
                      <Area type="monotone" dataKey="total" stroke="#58a6ff" fill="url(#gTotal)" strokeWidth={2} dot={false} />
                      <Area type="monotone" dataKey="info"  stroke="#3fb950" fill="url(#gInfo)"  strokeWidth={1} dot={false} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
